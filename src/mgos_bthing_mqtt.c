...
static struct mgos_bthing_mqtt_ctx *s_bthing_mqtt_ctx = NULL;
...
void mg_bthing_mqtt_on_set_state(struct mg_connection *nc, const char *topic,
  ...
  if (s_bthing_mqtt_ctx->sub_mode == MG_BTHING_MQTT_MODE_SINGLE && item) {
    if (!state) state = mgos_bvar_new_nstr(msg, msg_len);
    mgos_bthing_set_state(item->thing, state);
  } else if ((s_bthing_mqtt_ctx->sub_mode == MG_BTHING_MQTT_MODE_AGGREGATE) && state) {
    #ifdef MGOS_BTHING_MQTT_AGGREGATE_MODE
    const char *key_name;
...
static void mg_bthing_mqtt_on_created(int ev, void *ev_data, void *userdata) {
  ...
  if (s_bthing_mqtt_ctx->pub_mode == MG_BTHING_MQTT_MODE_SINGLE) {
  ...
  #if MGOS_BTHING_HAVE_ACTUATORS
  if (s_bthing_mqtt_ctx->sub_mode == MG_BTHING_MQTT_MODE_SINGLE) {
...
bool mgos_bthing_mqtt_init_context();
  ...
  s_bthing_mqtt_ctx = (struct mgos_bthing_mqtt_ctx*)calloc(1, sizeof(struct mgos_bthing_mqtt_ctx));
  
  s_bthing_mqtt_ctx->publishing = false;
  if (mg_bthing_scount(mgos_sys_config_get_bthing_mqtt_sub_topic(), MGOS_BTHING_ENV_THINGID) == 0) {
    #ifdef MGOS_BTHING_MQTT_AGGREGATE_MODE
    s_bthing_mqtt_ctx->sub_mode = MG_BTHING_MQTT_MODE_AGGREGATE;
    #else
    ...
    s_bthing_mqtt_ctx->sub_mode = MG_BTHING_MQTT_MODE_SINGLE;
  }
  if (mg_bthing_scount(mgos_sys_config_get_bthing_mqtt_pub_topic(), MGOS_BTHING_ENV_THINGID) == 0) {
    #ifdef MGOS_BTHING_MQTT_AGGREGATE_MODE
    s_bthing_mqtt_ctx->pub_mode = MG_BTHING_MQTT_MODE_AGGREGATE;
    #else
    ...
    s_bthing_mqtt_ctx->pub_mode = MG_BTHING_MQTT_MODE_SINGLE;
  }
...
bool mgos_bthing_mqtt_init() {
  ...
  if (s_bthing_mqtt_ctx->sub_mode == MG_BTHING_MQTT_MODE_AGGREGATE) {
